...
class SimpleTaxiEnv:
    ...
    def reset(self):
        ...
        available_locations = set((i, j) for i in range(self.grid_size) for j in range(self.grid_size))
        
        # Place random obstacles
        obstacle_count = random.randint(0, self.grid_size ** 2 // self.grid_min)
        self.obstacles = set(random.sample(list(available_locations), obstacle_count))
        
        # Remove obstacles from available locations
        available_locations -= self.obstacles
        
        # Place taxi
        self.taxi_pos = random.choice(list(available_locations))
        self.taxi_loc = self.taxi_pos
        
        # Place stations
        self.stations = random.sample(list(available_locations), 4)
